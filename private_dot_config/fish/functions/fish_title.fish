function fish_title
    ...
    if not set -q INSIDE_EMACS; or string match -vq '*,term:*' -- $INSIDE_EMACS
        ...
        # If we're in a container, we print the container name.
        set -q CONTAINER_ID
        and set ssh "["(echo $CONTAINER_ID | string sub -l 10 | string collect)"]"
        # Otherwise, print this machine's hostname.
        if not test -n "$ssh"
